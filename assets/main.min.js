...
table=$("#results table");e.sort(sort_stars_by_distance);$.each(e,function(f,g){$.each(g.sts,function(i,h){if(h.has_blackmarket){table.append('<tr class="success"><td>'+g.name+'</td><td><a href="http://eddb.io/system/'+h.id+'">'+h.name+"</a></td><td>"+($.fn.func=function(){if(h.allegiance==null){return"Unknown";
...
}else{return Math.round(h.distance_to_star)+" Ls";}})()+"</td><td>Yes</td>");}else{table.append("<tr><td>"+g.name+'</td><td><a href="http://eddb.io/system/'+h.id+'">'+h.name+"</a></td><td>"+($.fn.func=function(){if(h.allegiance==null){return"Unknown";
}else{return h.allegiance;}})()+"</td><td>"+Math.round10(distance_to_star($.bmdata.current_sy,g),-2)+" Ly</td><td>"+($.fn.func=function(){if(h.distance_to_star==null){return"--";
