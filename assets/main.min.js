...
table=$("#results table");e.sort(sort_stars_by_distance);$.each(e,function(f,g){$.each(g.sts,function(i,h){console.log("station: "+h);if(h.has_blackmarket){table.append('<tr class="success"><td>'+g.name+"</td><td>"+h.name+"</td><td>"+($.fn.func=function(){if(h.allegiance==null){return"Unknown";
...
}else{return Math.round(h.distance_to_star)+" Ls";}})()+"</td><td>Yes</td>");}else{table.append("<tr><td>"+g.name+"</td><td>"+h.name+"</td><td>"+($.fn.func=function(){if(h.allegiance==null){return"Unknown";
}else{return h.allegiance;}})()+"</td><td>"+Math.round10(distance_to_star($.bmdata.current_sy,g),-2)+" Ly</td><td>"+($.fn.func=function(){if(h.distance_to_star==null){return"--";
...
}function sy_within_radius(b,c,a){vector=Math.sqrt(Math.pow((c.x-b.x),2)+Math.pow((c.y-b.y),2)+Math.pow((c.z-b.z),2));if(a>vector&&c.id!=b.id){return true;
}return false;}function sy_sts_with_bm(d){var a=$.bmdata.sts_by_sy_id[d.id];var b=[];for(var c in a){st=a[c];if(st.has_blackmarket==true||st.has_blackmarket==null){b.push(st);
